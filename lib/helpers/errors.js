 ...
 * URL parsing to create the initial "state" of options for http.request(),
...
function fullRequestUrl(opts) {
    ...
    // This path value includes any query params as well. Combine it with the
    // existing request host options to get the full URL.
    var host = opts.host || opts.hostname || '';

    if (opts.port && host.indexOf(':') === -1) {
        host += ':' + opts.port;
    }

    return new URL(opts.path || '/', opts.protocol + '//' + host).href;
}
