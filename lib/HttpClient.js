        ...
        _rawRequest = function _wrappedRawRequest(_opts, _cb) {
            ...
            _before(opts, function _beforeCb(ctx) {
                ...
                rawRequest(opts, function _rawRequestCb(err, req) {
                    // We've now attempted to make the request. If we errored
                    // out immediately, there won't be a 'req' object, but we'll
                    // want to call the after hook with the error. If there is
                    // a req object, we'll call our after hook when that emits
                    // 'result' so that we have the 'res' paramter.
                    ...
                    if (req) {
                        ...
                        _after(err, undefined, undefined, ctx,
                            function _afterCb() {
                                // Do nothing for now
                            }
                        );
                        ...
                        assert.ok(false,
                            'rawRequest callback called w/o req or err');
                    }
