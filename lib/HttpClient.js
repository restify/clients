...
var KeepAliveAgent = http.Agent;
var KeepAliveAgentSecure = https.Agent;

// maxSockets defaults to Infinity, but that doesn't lend itself well to
// KeepAlive, since sockets will never be reused.
var httpMaxSockets = Math.min(http.globalAgent.maxSockets, 1024);
var httpsMaxSockets = Math.min(https.globalAgent.maxSockets, 1024);
...
function proxyOptsFromStr(str) {
    ...
    if (parsed.username || parsed.password) {
        proxyOpts.proxyAuth = decodeURIComponent(parsed.username);

        if (parsed.password) {
            proxyOpts.proxyAuth += ':' + decodeURIComponent(parsed.password);
        }
    }
...
HttpClient.prototype.basicAuth = function basicAuth(username, password) {
    if (username === false) {
        ...
        var buffer = Buffer.from(username + ':' + password, 'utf8');
...
HttpClient.prototype._options = function (method, options) {
    ...
    // Merge URL defaults into the request options IFF they haven't already
    // been set at verb time.
    if (this.url.href) {
        if (!opts.protocol) {
            opts.protocol = this.url.protocol;
        }

        if (!opts.hostname) {
            opts.hostname = this.url.hostname;
        }

        if (!opts.host) {
            opts.host = this.url.host;
        }

        if (!opts.port) {
            opts.port = this.url.port || null;
        }

        if (!opts.href) {
            opts.href = this.url.href;
        }

        if (!opts.auth && (this.url.username || this.url.password)) {
            opts.auth = decodeURIComponent(this.url.username);

            if (this.url.password) {
                opts.auth += ':' + decodeURIComponent(this.url.password);
            }
        }
    }
