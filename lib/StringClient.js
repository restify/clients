...
function StringClient(options) {
    ...
    assert.optionalBool(options.supportLatin1ContentMd5,
        'options.supportLatin1ContentMd5');
    ...
    if (!options.hasOwnProperty('supportLatin1ContentMd5')) {
        this.supportLatin1ContentMd5 = true;
    } else {
        this.supportLatin1ContentMd5 = options.supportLatin1ContentMd5;
    }
}
...
StringClient.prototype.parse = function parse(err, req, res, callback) {
    ...
    function done() {
        ...
        if (hash && md5 !== hash.digest('base64') &&
                (!binhash || md5 !== binhash.digest('base64'))) {
    ...
    if (md5 && req.method !== 'HEAD' && res.statusCode !== 206) {
        ...
        if (self.supportLatin1ContentMd5) {
            binhash = crypto.createHash('md5');
        }
    }
