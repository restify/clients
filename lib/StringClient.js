...
StringClient.prototype.parse = function parse(err, req, res, callback) {
    ...
    res.on('data', function onData(chunk) {
        if (hash) {
            // restify/clients#173: Accept a Content-MD5 calculated for
            // both a UTF-8 encoded body (the likely encoding for a
            // string body and the default for node v6+ servers) and a
            // latin-1 encoded body (the default, called 'binary'
            // encoding, in node v4 and below servers).
