...
function StringClient(options) {
    ...
    assert.optionalBool(options.allowAlternativeContentMd5,
        'options.allowAlternativeContentMd5');
    ...
    // restify/clients#173: When the allowAlternativeContentMd5 option is set,
    // the client will accept either a Content-MD5 header that matches a md5
    // hash of the UTF-8 encoded body (the likely encoding for a string body and
    // the default hash encoding for node v6+), or a md5 hash of the latin-1
    // encoded body (the default, called 'binary' encoding, in node v4 and
    // below).
    if (options.allowAlternativeContentMd5) {
        if (nodeMajorVer >= 6) {
            this.altContentMd5Encoding = 'latin1';
        } else {
            this.altContentMd5Encoding = 'utf8';
        }
    }
}
...
StringClient.prototype.parse = function parse(err, req, res, callback) {
    ...
    if (md5 && req.method !== 'HEAD' && res.statusCode !== 206) {
        // restify/clients#173: Accept a Content-MD5 calculated for both
        // a UTF-8 encoded body (the likely encoding for a string body
        // and the default for node v6+ servers) and a latin-1 encoded
        // body (the default, called 'binary' encoding, in node v4 and
        // below servers).
        hash = crypto.createHash('md5');

        if (self.altContentMd5Encoding) {
    ...
    res.on('data', function onData(chunk) {
        if (hash) {
            ...
            if (hash2) {
                hash2.update(chunk, self.altContentMd5Encoding);
            }
        }
}
